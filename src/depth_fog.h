...
inline void applyDepthFog(float nearClip, float farClip,
  ...
  for (unsigned i = 0; i < W * H; i += 8) {
    ...
    __m256 rOut = _mm256_fmadd_ps(pR, invFog, _mm256_mul_ps(vFogR, fog));
    __m256 gOut = _mm256_fmadd_ps(pG, invFog, _mm256_mul_ps(vFogG, fog));
    __m256 bOut = _mm256_fmadd_ps(pB, invFog, _mm256_mul_ps(vFogB, fog));
